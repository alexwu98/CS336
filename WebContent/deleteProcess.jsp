	...
	<%
		try {
			...
			if (stationID.isEmpty()) {
				...
				String checkOD = "SELECT t.origin, t.destination FROM Schedules s, TransitLines t WHERE s.schedule_id=? AND s.transit_name=t.transit_name";
				PreparedStatement ps3 = con.prepareStatement(checkOD);
				ps3.setString(1, scheduleID);
				ResultSet stations = ps3.executeQuery();
				if (stations.first() == true) {
					String origin = stations.getString("origin");
					String destination = stations.getString("destination");
					if (stationID.equals(origin) || stationID.equals(destination)) {
						response.sendRedirect("cannotDeleteDO.jsp");
					} else {
